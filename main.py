...
async def process_chat(
    ...
    try:
        ...
        # Extract images from formatted results if available
        images = []
        if formatted_results and 'image_results' in formatted_results:
            images = [
                {
                    'url': img.get('url', ''),
                    'title': img.get('title', '')
                }
                for img in formatted_results['image_results']
            ]
        
        return ChatResponse(
            response=ai_response,
            sources=[],  # TODO: Add source handling if needed
            images=images,
